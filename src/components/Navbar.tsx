    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-200 ${
      ...
      <div className="max-w-content mx-auto px-5 md:px-8 flex items-center justify-between h-16">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          {loggedIn ? (
            <Link href="/dashboard" className="text-sm font-semibold text-white bg-deep-blue border-2 border-deep-blue rounded-lg px-5 py-2 hover:bg-deep-blue/90 transition-colors flex items-center gap-2">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="lg:hidden bg-white border-t border-neutral-100 overflow-hidden">
            <div className="px-5 py-4 space-y-1">
              ...
              <div className="pt-3 border-t border-neutral-100 space-y-2">
                {loggedIn ? (
                  <Link href="/dashboard" onClick={() => setMobileOpen(false)} className="block text-center text-sm font-semibold text-white bg-deep-blue border-2 border-deep-blue rounded-lg px-5 py-2.5 transition-colors">{userName} — Dashboard</Link>
                ) : (
